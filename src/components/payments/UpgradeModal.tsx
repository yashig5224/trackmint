...
export function UpgradeModal({ open, onOpenChange, feature, tier = "pro" }: Props) {
  ...
  const priceId = plan.priceIdMonthly!;

  const handleUpgrade = () => {
    if (!user) {
      setPendingCheckout({ priceId, planName: plan.name, cycle: "monthly" });
      onOpenChange(false);
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden bg-white border-0 shadow-2xl">
          <div className="relative p-8">
            ...
            <p className="text-sm text-slate-500 mt-2">
              ₹{plan.priceMonthly}/month — cancel anytime.
            </p>
      ...
      <CheckoutDialog
        ...
        priceId={checkoutOpen ? priceId : null}
        planName={plan.name}
      />
    </>
  );
